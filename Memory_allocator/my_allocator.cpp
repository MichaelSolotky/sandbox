...
static const unsigned mask = 0xfffffffe;
static const unsigned size_of_heap = 4294967280;
static const unsigned minimal_appropriate_size = sizeof(unsigned) << 2;
static void *head_of_memory = NULL;
static void *tmp_mem = NULL;

typedef class my_allocator {
    void init();
...
void my_allocator :: init() {
    if(head_of_memory)
        tmp_mem = mem = head_of_memory;
    else
        tmp_mem = mem = head_of_memory = malloc((size_t) size_of_heap + (sizeof(unsigned) << 2));
...
void my_allocator :: cut_a_block(char *ptr, unsigned n) {
...
void *my_allocator :: merge_blocks(unsigned n) {
    void *tmp_res;
...
void *my_allocator :: my_new(unsigned n) {
    n = (n & mask) + ((n & 1) ? 2 : 0);
...
void my_allocator :: my_delete(void *ptr) {
    if(!ptr)
        return;
...
void *my_allocator :: my_realloc(void *ptr, unsigned new_size) {
...
void my_allocator :: testing_output(void *begin, void *end) {
...
/*int main() {
    my_allocator *A = new my_allocator;
    unsigned n = 2;
    scanf("%u", &n);
    A->testing_output(tmp_mem, (void *) ((char *) tmp_mem + size_of_heap));
    void *ptr = A->my_new(n);
    void *ptr1 = A->my_new(n);
    void *ptr2 = A->my_new(n);
    void *ptr3 = A->my_new(n);
    A->testing_output(tmp_mem, (void *) ((char *) tmp_mem + size_of_heap));
    ptr = A->my_realloc(ptr, n + 32);
    A->testing_output(tmp_mem, (void *) ((char *) tmp_mem + size_of_heap));
    A->my_delete(ptr);
    A->my_delete(ptr1);
    A->my_delete(ptr2);
