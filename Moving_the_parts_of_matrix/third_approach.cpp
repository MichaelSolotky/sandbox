...
void processing(node *arr, int q, int m) {
	...
	for(int i = 0; i < q; i++) {
		...
		find_positions(up_src_1, up_src_2, dw_src_1, dw_src_2, lf_src_1, lf_src_2, rg_src_1, rg_src_2, arr, a, b, w, m, h);
		find_positions(up_dst_1, up_dst_2, dw_dst_1, dw_dst_2, lf_dst_1, lf_dst_2, rg_dst_1, rg_dst_2, arr, c, d, w, m, h);
		for(int i = 0; i < h; i++) {
